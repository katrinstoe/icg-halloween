...
export default class RayVisitor implements Visitor {
    ...
    render(
        rootNode: Node,
        camera: { origin: Vector, width: number, height: number, alpha: number, shininess: number, kS: number, kD: number},
            ...
            for (let y = 0; y < height; y++) {
                ...
                if (this.intersection) {
                    if (!this.intersectionColor) {
                        ...
                        let color = phong(this.intersectionColor, this.intersection, lightPositions, camera.shininess, camera.origin, camera.kS, camera.kD);
